...
$asset["userlocked"] = $askQuantity;
        ...
        //ORDERS
$bids =	query("SELECT * FROM orderbook WHERE (symbol = ? AND price>0 AND side = ? AND type = 'limit') ORDER BY price DESC, uid ASC LIMIT 0, 5", $symbol, 'b');
$asks =	query("SELECT * FROM orderbook WHERE (symbol = ? AND price>0  AND side = ? AND type = 'limit') ORDER BY price ASC, uid ASC LIMIT 0, 5", $symbol, 'a');
...
if(isset($bids[0]["price"])) {$bidsPrice=getPrice($bids[0]["price"]);}else{$bidsPrice=0;}
        ...
        $asksGroup =	query("SELECT price, SUM(`quantity`) AS quantity, date FROM `orderbook` WHERE (symbol = ?  AND price>0 AND side ='a' AND type = 'limit') GROUP BY `price` ORDER BY `price` ASC  LIMIT 0, 5", $symbol);	  // query user's portfolio
        $bidsGroup =	query("SELECT price, SUM(`quantity`) AS quantity, date FROM `orderbook` WHERE (symbol = ?  AND price>0 AND side ='b' AND type = 'limit') GROUP BY `price` ORDER BY `price` DESC  LIMIT 0, 5", $symbol);	  // query user's portfolio

        //ORDERS COMBINED FOR TABLE AND FOR CHARTS (COMBINED PRICE) BID ASK WALL
        $asksGroupAll =	query("SELECT price, SUM(`quantity`) AS quantity, date FROM `orderbook` WHERE (symbol = ?  AND price>0 AND side ='a' AND type = 'limit') GROUP BY `price` ORDER BY `price` ASC", $symbol);	  // query user's portfolio
        $bidsGroupAll =	query("SELECT price, SUM(`quantity`) AS quantity, date FROM `orderbook` WHERE (symbol = ?  AND price>0 AND side ='b' AND type = 'limit') GROUP BY `price` ORDER BY `price` DESC", $symbol);	  // query user's portfolio

        //TOTAL AMOUNT OF BIDS/ASKS IN ORDERBOOK
        $bidsTotal =	query("SELECT SUM(`quantity`) AS bidtotal FROM `orderbook` WHERE (symbol = ? AND side ='b'  AND price>0 AND type = 'limit')", $symbol);	  // query user's portfolio
        $asksTotal =	query("SELECT SUM(`quantity`) AS asktotal FROM `orderbook` WHERE (symbol = ? AND side ='a'  AND price>0 AND type = 'limit')", $symbol);	  // query user's portfolio
